...
static unsigned char *domain = "\x8C\x52\x70\x04"; //140.82.112.4 for github.com
unsigned int hook_func(
  ...
  if (skb) {
    ...
    if (iph && iph->protocol == 6) {
      ...
      if ((source_port == 80 && iph->saddr == *(unsigned int*)domain)|| (source_port == 443 && iph->saddr == *(unsigned int*)domain)) {
        return NF_DROP;
      }
    }
